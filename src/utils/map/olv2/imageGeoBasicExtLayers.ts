...
export default class OlGeoImageBasicExtLayers {
  ...
  public async createLayer(
    ...
    if (options.level) {
      ...
      // const lbToRuBbox = getLbToRuCoordinates(options.bbox);
      // console.log("lbToRuBbox1", lbToRuBbox);
      // rect = lbToRuBbox[0];
      rect = options.bbox[0];
    }
    ...
    // const lbToRuBbox = getLbToRuCoordinates(options.bbox);
    // console.log("lbToRuBbox", lbToRuBbox);
    // const lbpixels = this.handle.getPixelFromCoordinate(transformTo3857(lbToRuBbox[0][0]));
    // const rbpixels = this.handle.getPixelFromCoordinate(transformTo3857(lbToRuBbox[0][1]));
    // const lupixels = this.handle.getPixelFromCoordinate(transformTo3857(lbToRuBbox[0][3]));

    // const lbtorbPixels = Math.pow(Math.pow(rbpixels[0] - lbpixels[0], 2) + Math.pow(rbpixels[1] - lbpixels[1], 2), 0.5);
    // const lbtolupixels = Math.pow(Math.pow(lupixels[0] - lbpixels[0], 2) + Math.pow(lupixels[1] - lbpixels[1], 2), 0.5);

    // const wScale = lbtorbPixels / imgMeta.width;
    // const hScale = lbtolupixels / imgMeta.height;
    ...
    // console.log("NND", imageOptions, imgMeta, widthInMeters, heigthInMeters, rect);
    // console.log("NND", imageOptions, imgMeta, lbtorbPixels, lbtolupixels);
