...
export default class OlBase {
  ...
  private originView: View | null = null;

  private constrainedView: View | null = null;
  ...
  public setConstrainedView(isConstrained = true) {
    if (this.handle) {
      if (isConstrained) {
        console.log("constrained View", isConstrained);
        this.constrainedView = new View({
          // extent: mapExtent,  // 用来限制显示区域
          // projection: 'EPSG:4326',
          center: transformLongitudeLatitude(this.getViewCenter()),
          zoom: this.getViewZoom(),
          minZoom: this.minLevel,
          maxZoom: this.maxLevel,
          extent: transformExtentTo3857([-177, -84.5, 175, 84.5]),
          // enableRotation: false,
        });
        this.handle.setView(this.constrainedView);
      } else {
        if (this.originView) {
          console.log("origin View", isConstrained);
          this.handle.setView(this.originView);
        }
      }
      this.viewHandle = this.handle.getView();
    }
