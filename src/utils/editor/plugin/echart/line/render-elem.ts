...
function renderEchartLine(elem: SlateElement, children: VNode[] | null, editor: IDomEditor): VNode {
  ...
  // DomEditor.getSelectedNodeByType(editor, wangEditorEchartLineType)
    ...
    hook: {
      insert: (vnode: any) => {
        ...
        if (id) {
          ...
          if (newOptions) {
            echartIns.renderEchart(EchartsOptionBasePlus, newOptions, editor, selected);
          } else {
  ...
  console.log("renderEchartLine", vnode);
