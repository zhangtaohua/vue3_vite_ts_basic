...
const echartsId = ref("echarts_line_" + nanoid(10));
const echartsOptions = reactive({
  isHasData: true,
  parentId: "",
  options: lodash.cloneDeep(simpleLineOptions),
});
...
function updateEchartsOptions(newLineData: any, oldLineData: any) {
  ...
  if (isNeedRePaint && newLineData.node.chart) {
    const { line_type, xaxis_name, xaxis_data, xaxis_unit, yaxis } = newLineData.node.chart;
