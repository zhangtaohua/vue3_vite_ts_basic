...
import { nofityReportsSave } from "@/composables/reportsEditStatus";
...
const useCreateTemplateStore = useCreateReportTemplateStore();
...
const currentReportsName = computed(() => {
  if (
    useCreateTemplateStore.reportsTemplate &&
    useCreateTemplateStore.reportsTemplate.info &&
    useCreateTemplateStore.reportsTemplate.info.templateName
  ) {
    return useCreateTemplateStore.reportsTemplate.info.templateName;
  } else {
    return "";
  }
});
...
const isEidtReport = computed(() => {
  if (
    useCreateTemplateStore.reportsTemplate &&
    useCreateTemplateStore.reportsTemplate.info &&
    useCreateTemplateStore.reportsTemplate.info.isEdit
  ) {
    return true;
  } else {
    return false;
...
function fixRefreshBug() {
  ...
  if (route.name == "eidtTemplate" || route.path == "/edit/template") {
    if (
      !useCreateTemplateStore.reportsTemplate ||
      !useCreateTemplateStore.reportsTemplate.info ||
      !useCreateTemplateStore.reportsTemplate.info.isEdit
...
function initCreateEditData() {
  if (useCreateTemplateStore.reportsTemplate && useCreateTemplateStore.reportsTemplate.info) {
    // 1 如果是编辑报告 加载要编辑的报告
    if (useCreateTemplateStore.reportsTemplate.info.isEdit) {
      console.log("编辑初始化");
      // 获取原模板 并 加载数据
      if (useCreateTemplateStore.reportsTemplate.info.templateUuid) {
        const templateUuid = useCreateTemplateStore.reportsTemplate.info.templateUuid;
        // 新接口，获取模板数据也用此接口
    ...
    } else if (useCreateTemplateStore.reportsTemplate.info.isCreate) {
      // 2 如果是创建报告
      if (useCreateTemplateStore.reportsTemplate.info.refReportUuid) {
        // 2.1 创建报告时 是基于旧有报告
        // 此时加载旧有报告 并 加载数据
        console.log("创建初始化： 基于报告");
        const reportUuid = useCreateTemplateStore.reportsTemplate.info.refReportUuid;
        getReportByUuidRequest(reportUuid);
        nofityReportsSave(false);
      } else if (useCreateTemplateStore.reportsTemplate.info.refTemplateUuid) {
        // 2.2 创建报告时 是基于报告模板
        // 此时加载报告模板 并 加载数据
        console.log("创建初始化： 基于模板");
        const refTemplateUuid = useCreateTemplateStore.reportsTemplate.info.refTemplateUuid;
        // 新接口，获取模板数据也用此接口
        getReportByUuidRequest(refTemplateUuid);
        nofityReportsSave(false);
      } else {
...
function submitHandle() {
  return new Promise((resolve: any, reject: any) => {
    ...
    const createReportsInfo = useCreateTemplateStore.reportsTemplate.info;
    console.log("submitHandleTemp", renderData, createReportsInfo);
    if (useCreateTemplateStore.reportsTemplate && useCreateTemplateStore.reportsTemplate.info) {
      const reqData: any = {
        ...
        reportTemplateUuid: createReportsInfo.refTemplateUuid || "",
        reportTemplateName: createReportsInfo.refTemplateName || "",
        name: createReportsInfo!.templateName,
        uuid: createReportsInfo!.templateUuid || createedReportInfoUuid,
        status: 1, // 1启用 0禁用
        type: createReportMode.template,
      };
      // 1 如果是编辑报告 加载要编辑的报告
      if (useCreateTemplateStore.reportsTemplate.info.isEdit) {
        console.log("编辑模板 开始保存");
...
const editReportRequest = (reqData: any) => {
  return new Promise((resolve: any, reject: any) => {
    editReportData(reqData)
      .then((res: any) => {
        if (res.code == 0) {
          console.log("编辑模板 成功", res);
          // 通知保存成功
          nofityReportsSave(true);
          message.success("保存成功！", 2);
          return resolve(true);
        } else {
          console.log("编辑模板 失败", res);
          nofityReportsSave(false);
          message.success("保存失败，请重试", 2);
          return reject(false);
        }
...
function tipsForSaveCancelHandle() {
  if (isFromMenuChange) {
    ...
    // 这里要想办法通知继续跳转。
    nofityReportsSave(true);
  } else if (useCreateTemplateStore.reportsTemplate && useCreateTemplateStore.reportsTemplate.info) {
...
function cleanUseResouce() {
  ...
  // 清空 编辑 创建报告 初始数据
  useCreateTemplateStore.resetCreateReportTemplateInfo();
}
</script>
