<template>
  <div class="col_nw_fs_center wh_100p_100p">
    <div class="row_nw_center_center rj_rpt_line_b">
      <a-button type="primary" size="large" @click="downDataModule('pieModule.csv')">
    ...
    <div class="row_nw_fs_center rj_rpt_line_t">
      <span class="row_nw_fs_center rj_rpt_line_tl">饼图类型:</span>
      <a-radio-group v-model:value="echartsData.chart.pie_type" :options="simplePieTypeOptions" />
    </div>
    <div class="h_8r"></div>
    ...
    >
      <div class="row_nw_fs_center rj_rpt_line_t">
        <span class="row_nw_fs_center rj_rpt_line_tl">{{ `第${index + 1}名称:` }}</span>
        <a-input v-model:value="echartsData.chart.dataset[index].name" placeholder="请输入" />
      </div>
      <div class="row_nw_fs_center rj_rpt_line_t">
        <span class="row_nw_fs_center rj_rpt_line_tl">{{ `第${index + 1}数据:` }}</span>
        <a-input v-model:value="echartsData.chart.dataset[index].value" placeholder="请输入" />
      </div>
      <div class="h_8r"></div>
    </div>
    <div class="row_nw_fs_center rj_rpt_line_t">
...
import { EchartsPieType, simplePieTypeOptions } from "@/common/echarts/pieOptions";
...
function setCsvDataToEchartsData(pieRecords: Array<any>) {
  if (pieRecords.length) {
    const chartsTemp = {
      pie_type: echartsData.chart.pie_type,
      dataset: [],
    };
    for (let i = 0; i < pieRecords.length; i++) {
      if (pieRecords[i][0] == "") {
        ...
        // 从第四行开始处理
        if (i >= 3) {
          const nameTemp = pieRecords[i][0];
          const valueTemp = pieRecords[i][1];
          const ydataTemp = {
            name: nameTemp,
            value: valueTemp,
          };
...
function initOptions() {
  ...
  console.log("isRefresh pie", vnode);
  const node = vnode.node;
  nextTick(() => {
    echartsData.title = node.title || "";
    echartsData.name = node.name || "";
    if (node.chart) {
      echartsData.chart.dataset = node.chart.dataset || [{ name: "", value: "" }];
    }
