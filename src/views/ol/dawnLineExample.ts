...
export default class OlDawnLineHelper extends OlBase {
  ...
  public DawnLineIns: OlDawnLineLayers | null = null;
  ...
  constructor(target: string, pixelRatio = 2) {
    ...
    this.DawnLineIns = new OlDawnLineLayers(this);
  ...
  public addDawnLineLayer(options: DawnLineOptions) {
    if (options) {
      const isAdded = this.DawnLineIns!.addLayer(options);
      if (isAdded) {
        this.__funcLayers.set(options, this.DawnLineIns);
      }
    }
  }

  public removeDawnLineLayer(options: DawnLineOptions) {
    if (options) {
