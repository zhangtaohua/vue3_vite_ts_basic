...
import CsSingleImageLayers from "@/utils/map/cesium/singleImageLayers";
import type { SingleImageOptions } from "@/utils/map/cesium/singleImageLayersTypes";
...
export default class CsMapHelper extends CsBase {
  ...
  public csSingleImgIns: CsSingleImageLayers | null = null;
  ...
  constructor(target: string, csBasicOptions: CesiumBasicOptions) {
    ...
    this.csSingleImgIns = new CsSingleImageLayers(this);
  }
  ...
  public destructor() {
    ...
    this.csSingleImgIns!.destructor();
  ...
  public addTargetModel(imageOpt: SingleImageOptions) {
    if (this.csSingleImgIns) {
      this.csSingleImgIns.addLayer(imageOpt);
    }
  }

  public removeTargetModel(imageOpt: SingleImageOptions) {
    if (this.csSingleImgIns) {
      this.csSingleImgIns.removeLayer(imageOpt);
    }
  }

  public clearTargetModel() {
    if (this.csSingleImgIns) {
      this.csSingleImgIns.clearLayer();
    }
  }
}
